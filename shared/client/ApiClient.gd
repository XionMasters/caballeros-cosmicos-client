...
func _start_image_request(req: Dictionary) -> void:
	...
	# Construir URL basada en el tipo de ruta
	var url: String
	if req.path.begins_with("/assets/"):
		# Rutas de assets: no incluyen /api
		url = base_url.trim_suffix("/api") + req.path
	else:
		# Rutas de API: incluyen /api
