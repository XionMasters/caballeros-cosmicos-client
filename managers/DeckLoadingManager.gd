...
func _fetch_deck_cards(deck_id: String) -> void:
	...
	print("[DeckLoadingManager] Fetching cards from deck: %s" % deck_id)
	
	var callback = func(success: bool, data: Variant, error: String) -> void:
		if not success:
			print("[DeckLoadingManager] Error cargando cartas: %s" % error)
			_finish_loading()
			return
		...
		if data is Array:
			...
			cards_array = data.get("data", [])
		
		print("[DeckLoadingManager] Cargadas %d cartas" % cards_array.size())
		
		# Convertir a CardData
		loaded_cards.clear()
		_total_cards_to_load = cards_array.size()
		_images_loaded = 0
...
func _on_deck_cards_loaded(_result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray, http: HTTPRequest) -> void:
	"""Procesar cartas cargadas - DEPRECATED: usar ApiClient ahora"""
	http.queue_free()
